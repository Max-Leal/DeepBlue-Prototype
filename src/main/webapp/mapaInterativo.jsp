     ...
     // Adicione os marcadores dinamicamente com filtro da query
        ...
        <% for (Local local : locais) { %>
            L.marker([<%= local.getLatitude() %>, <%= local.getLongitude() %>])
                .addTo(map)
                .bindPopup('<b><%= local.getNome() %></b><br><%= local.getDescricao() %><br><a href="local-detalhe.jsp?id=<%= local.getId() %>">Ver detalhes</a> <br> <a href="https://www.google.com/maps?q=<%=local.getLatitude()%>,<%=local.getLongitude()%>"</a>Google Maps');
        <%}%>
    </script>
